...
import robomimic
from robomimic.models.obs_core import CropRandomizer, RNN_Base, Randomizer
import torch.distributions as D

import robomimic.utils.obs_utils as ObsUtils
import agents.models.robomimic.utils.torch_utils as TorchUtils
...
class SpatialSoftmax(torch.nn.Module):
    def __init__(self, in_c, in_h, in_w, num_kp=None):
        ...
        self._spatial_conv = torch.nn.Conv2d(in_c, num_kp,
                                             kernel_size=1)  # Kernel size ==1 h, w keep constant, only change the channels
    ...
    def forward(self, x):  # B C H W
        ...
        if self._num_kp != self._in_c:
            h = self._spatial_conv(h)  # B num_kp H W
        h = h.contiguous().view(-1,
                                self._in_h * self._in_w)  # tensor.contiguous() ensures that the tensor is stored in a contiguous block of memory| h -> B*num_kp, H*W
        attention = F.softmax(h,
                              dim=-1)  # softmax would be applied along the last dimension, which corresponds to the spatial dimensions of the feature map

        keypoint_x = torch.sum(self.pos_x * attention, dim=1, keepdim=True).view(-1,
                                                                                 self._num_kp)  # (B*num_kp,1) --> (B, num_kp)
        keypoint_y = torch.sum(self.pos_y * attention, dim=1, keepdim=True).view(-1, self._num_kp)

        keypoints = torch.cat([keypoint_x, keypoint_y], dim=1)  # (B, 2*num_kp)
        return keypoints
...
class ResnetConv(torch.nn.Module):
    ...
    def output_shape(self, input_shape):
        ...
        if self.no_stride:
            scale = scale / 4.
        out_h = int(math.ceil(input_shape[1] / scale))  # 返回的是大于或等于H / scale的数值
...
class Norm(nn.Module):
    def __init__(self, dim):
        ...
        For batch of images, the dim should be the list of last three dimensions e.g. dim = [C, H, W]
        If the dim is a integer, it should do normalizaion only on the last dimension of the input tensor"""
...
class TransformerDecoder(nn.Module):
    ...
    def compute_mask(self, input_shape):
        if self.num_elements is None or self.seq_len is None or self.num_elements != input_shape[2] or self.seq_len != \
                input_shape[1]:
            self.seq_len = input_shape[1]
            self.num_elements = input_shape[2]
            self.original_mask = (
                        torch.triu(torch.ones(self.seq_len, self.seq_len)) - torch.eye(self.seq_len, self.seq_len)).to(
                DEVICE)
            self.mask = self.original_mask.repeat_interleave(self.num_elements, dim=-1).repeat_interleave(
...
class RoIAlignSpatialSoftmaxWrapper(nn.Module):
    def __init__(self,
        ...
        self.roi_align = torchvision.ops.RoIAlign(output_size=output_size,
                                                  ...
                                                  sampling_ratio=sampling_ratio,
                                                  aligned=aligned)
        self.spatial_softmax = SpatialSoftmax(in_c=input_shape[0], in_h=output_size[0], in_w=output_size[1],
                                              num_kp=num_kp)
        self.num_kp = num_kp
...
class TranslationAug(nn.Module):
    ...
    def __init__(
        ...
        pad_output_shape = (
        input_shape[0], input_shape[1] + translation, input_shape[2] + translation)  # channel, h+padding, w+padding
    ...
    def forward(self, x):  # B C H W
        if self.training:
            out = self.pad(
                x)  # first padding, set the crop_heigth and crop_weight are equal to original height and weight
            out = self.crop_randomizer.forward_in(out)  # BN C CROP_H CROP_W -- N is the number of crops
        else:
            out = x
        return out
...
class TranslationAugGroup(nn.Module):
    ...
    def __init__(
        ...
        original_input_shapes = input_shapes[0]
        pad_output_shape = (sum(self.channels), original_input_shapes[1] + translation,
                            original_input_shapes[2] + translation)  # after padding: C H+trans W+trans

        self.crop_randomizer = CropRandomizer(input_shape=pad_output_shape,
                                              crop_height=original_input_shapes[1],
                                              crop_width=original_input_shapes[2])

    def forward(self, x_groups):  # input: (agentview_rgb, eye_in_hand)  image group  (B C H W, B C H W)
        if self.training:
            x = torch.cat(x_groups, dim=1)  # B 2C H W
            out = self.pad(x)  # B 2C H+translate  W+translate
            out = self.crop_randomizer.forward_in(out)  # BN 2C H W -- N is the number of  crops-- hre is 1
            out = torch.split(out, self.channels, dim=1)  # (BN C H W, BN C H W)
            return out
        else:
            out = x_groups
        return out
...
class ShiftBBoxTranslationAugGroup(nn.Module):
    ...
    def forward(self, x_groups):
        if self.training:
            ...
            shift_pixels = torch.cat(
                (crop_inds[..., 1:2], crop_inds[..., 0:1], crop_inds[..., 1:2], crop_inds[..., 0:1]),
                dim=-1) - self.pad_translation
...
class TemporalCropRandomizer(Randomizer):
    ...
    def sample_temporal_random_image_crops(self, images, crop_height, crop_width, num_crops, pos_enc=False):
        ...
        crop_inds_h = (max_sample_h * torch.rand(*source_im.shape[:-4], num_crops).to(device)).long().expand(-1,
                                                                                                             source_im.shape[
                                                                                                                 -4])
        crop_inds_w = (max_sample_w * torch.rand(*source_im.shape[:-4], num_crops).to(device)).long().expand(-1,
                                                                                                             source_im.shape[
                                                                                                                 -4])
...
class FlattenProjection(nn.Module):
    def __init__(self,
        ...
        assert (
                    len(input_shape) == 4), "You should not use FlattenProjection if not having a tensor with 4 dimensions (excluding batch dimension)"  # B C H W
...
class SpatialProjection(nn.Module):
    def __init__(self,
        ...
        assert (
                    len(input_shape) == 3), "You should not use FlattenProjection if not having a tensor with 3 dimensions (excluding batch dimension)"

        in_c, in_h, in_w = input_shape
        num_kp = out_dim // 2
        self.out_dim = out_dim
        self.spatial_softmax = SpatialSoftmax(in_c, in_h, in_w, num_kp=num_kp)
        self.projection = nn.Linear(num_kp * 2, out_dim)  # num_kp *2 == out_dim

    def forward(self, x):
        out = self.spatial_softmax(x)  # a set of keypoint [kx1, ky1, ... kxn,kyn]
        out = self.projection(out)
        return out
...
class EyeInHandKeypointNet(nn.Module):
    def __init__(self,
        ...
        self.spatial_softmax = SpatialSoftmax(in_c=encoder_output_shape[0], in_h=encoder_output_shape[1],
                                              in_w=encoder_output_shape[2], num_kp=num_kp)
...
class BCCatGroupModalities(torch.nn.Module):
    def __init__(self,
        ...
        self.num_modalities = int(self.use_eye_in_hand) + int(self.use_joint) + int(self.use_gripper) + int(
            self.use_ee) + int(self.use_gripper_history)
...
class CatSeqGroupModalities(torch.nn.Module):
    def __init__(self,
        ...
        if self.learnable_position:
            self.learnable_position_dict = {
                "input_tensor": nn.Parameter(torch.randn(1, self.learnable_position_size)).to(DEVICE)}
            for obs_key in self.nets.keys():
                self.learnable_position_dict[obs_key] = nn.Parameter(torch.randn(1, self.learnable_position_size)).to(
                    DEVICE)
    ...
    def __init__(self,
        ...
        if self.use_eye_in_hand:
            self.nets["eye_in_hand_rgb"] = EyeInHandKeypointNet(visual_feature_dimension=embedding_size,
                                                                **kwargs["eye_in_hand"])
        if self.use_joint:
            self.nets["joint_states"] = ProprioProjection(input_shape=(joint_states_dim,), out_dim=embedding_size,
                                                          squash=squash)

        if self.use_gripper:
            self.nets["gripper_states"] = ProprioProjection(input_shape=(gripper_states_dim,), out_dim=embedding_size,
                                                            squash=squash)

        if self.use_gripper_history:
            self.nets["gripper_history"] = ProprioProjection(input_shape=(gripper_history_dim,), out_dim=embedding_size,
                                                             squash=squash)
        if self.use_ee:
...
class DataAugGroup(torch.nn.Module):
    ...
    def forward(self, x_groups):
        ...
        for i in range(len(x_groups)):
            split_channels.append(x_groups[i].shape[0])  # each image in x_groups with the shape B C H W
        if self.training:
            x = torch.cat(x_groups, dim=0)
            out = self.transforms(x)  # include color jitter and random erasing
            out = torch.split(out, split_channels, dim=0)  # each image would be splitted into original shape
            return out  # (processed_im1, processed_im2, .. )
        else:
            out = x_groups
        return out
...
class RPNPretrained(torch.nn.Module):
    def __init__(self,
        ...
        super().__init__()
        from crop_target_object import load_centernet_rpn

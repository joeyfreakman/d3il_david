...
class CausalSelfCrossAttentionV1(nn.Module):
    def __init__(self, n_embd, cross_embed, n_heads, attn_pdrop, resid_pdrop, block_size):
        ...
        self.beta = torch.nn.Parameter(torch.tensor(0.0))  # Initialize at 0 for sigmoid to start at 0.5
        self.mamba = Mamba(
    ...
    def forward(self, x, cross_input=None):
        ...
        if cross_input is not None:
            ...
            y_cross = y_cross.transpose(1, 2).contiguous().view(B, T, C)
...
class CausalSelfCrossAttentionV2(nn.Module):
    def __init__(self, n_embd, cross_embed, n_heads, attn_pdrop, resid_pdrop, block_size):
        ...
        self.beta = torch.nn.Parameter(torch.tensor(0.0))  # Initialize at 0 for sigmoid to start at 0.5
        self.alpha = torch.nn.Parameter(torch.tensor(0.0))  # Initialize at 0 for sigmoid to start at 0.5
        self.mamba1 = Mamba(
                            d_model=n_embd,  # Model dimension d_model
                            d_state=n_embd,  # SSM state expansion factor
                            d_conv=4,    # Local convolution width
    ...
    def forward(self, x, cross_input=None):
        ...
        beta = torch.sigmoid(self.beta)
        y_mamba1 = self.mamba1(x)

        y = y.transpose(1, 2).contiguous().view(B, T, C)
        y = beta * y + (1-beta) * y_mamba1
        ...
        if cross_input is not None:
            ...
            mamba_input = torch.cat([cross_input, x], dim=1)
            alpha = torch.sigmoid(self.alpha)
            y_mamba2 = self.mamba2(mamba_input)[:, T_C:, :]

            y_cross = alpha * y_cross + (1-alpha) * y_mamba2
            # combine self-attention and cross-attention
            y = y + y_cross  # or any other combination strategy
...
class CausalSelfCrossAttentionV3(nn.Module):
    def __init__(self, n_embd, cross_embed, n_heads, attn_pdrop, resid_pdrop, block_size):
        super().__init__()
    ...
    def forward(self, x, cross_input=None):
        ...
        if cross_input is not None:
            ...
            mamba_input = torch.cat([cross_input, x], dim=1)
            alpha = torch.sigmoid(self.alpha)
            y_mamba = self.mamba(mamba_input)[:, T_C:, :]

            y_cross = alpha * y_cross + (1 - alpha) * y_mamba
            # combine self-attention and cross-attention
            y = y + y_cross  # or any other combination strategy
...
class CausalSelfCrossAttentionV4(nn.Module):
    def __init__(self, n_embd, cross_embed, n_heads, attn_pdrop, resid_pdrop, block_size):
        super().__init__()
...
class DecoderBlockV1(nn.Module):
    ...
    def __init__(
        ...
        self.attn = CausalSelfCrossAttentionV1(
            n_embd,
            cross_embd,
            n_heads,
            attn_pdrop,
...
class DecoderBlockV3(nn.Module):
    ...
    def __init__(
        ...
        self.ln1 = LayerNorm(n_embd, bias=False)
        self.ln2 = LayerNorm(n_embd, bias=False)
        self.attn = CausalSelfCrossAttentionV3(
            n_embd,
            cross_embd,
            n_heads,
            attn_pdrop,
...
class DecoderBlockV4(nn.Module):
    ...
    def __init__(
        ...
        self.ln1 = LayerNorm(n_embd, bias=False)
        self.ln2 = LayerNorm(n_embd, bias=False)
        self.attn = CausalSelfCrossAttentionV4(
            n_embd,
            cross_embd,
            n_heads,
            attn_pdrop,
...
class TransformerDecoderV4(nn.Module):
